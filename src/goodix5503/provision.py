...
There is deliberately no CLI entry point. The only mutating operation accepts
one fixed 96-byte white-box value and emits the official 0xbb010003 TLV with
opcode 0xe0. Firmware, registers and configuration are not writable here.
"""
...
def provision_prepared_pairing(
    *, confirmation: str, timeout_seconds: float = 5.0
    ...
    """Perform one reviewed write and exact readback; requires later authorization."""
    ...
    try:
        ...
        if close_error is not None:
            raise ProvisioningError("failed to close provisioning USB session") from close_error
